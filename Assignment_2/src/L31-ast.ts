...
;;         |  (cond <cond-clause>+ <else-clause>)    / CondExp(cond-clause: CondClause[], else-clause: CExp)
...
;;                (<sexp>+ . <sexp>) | ( <sexp>* )       ##### L31
;; <cond-clause> ::= (<cexp> <cexp>)        / CondClause(test: CExp, then: CExp)
;; <else-clause> ::= (else <cexp>)          / ElseClause(alt: CExp)
*/
...
export type ElseClause = { tag: "ElseClause", alt: CExp; }
...
export const makeElseClause = (alt: CExp) : ElseClause =>
    ({ tag: "ElseClause", alt: alt});
...
const parseCondExp = (params: Sexp[]): Result<CondExp> => {
    if (params.length < 2) {
        return makeFailure("Invalid cond expression, at least one cond-clause and an else-clause required");
    }
    
    const condClauses : Sexp[] = params.slice(0, -1);
    const elseClauseSexp = params[params.length - 1];

    if(isArray(elseClauseSexp)){
        if(!(elseClauseSexp[0] === 'else' && elseClauseSexp.length === 2)){
            return makeFailure("Invalid else clause");
        }
    }
    
    const clausesResult : Result<CondClause[]> = 
        mapResult(
            (clause : Sexp) => {
                if(isArray(clause) && clause.length === 2) {
                    return safe2((test: CExp, then: CExp) => makeOk(makeCondClause(test, then)))
                        (parseL31CExp(first(clause)), parseL31CExp(second(clause)))
                } else {
                    return makeFailure("Invalid cond clause")
                }
            
            }, condClauses);

    const elseClauseResult = (parseElseExp(elseClauseSexp));
    const parsedelseClauseResult = bind(elseClauseResult, (elseClause: Sexp) => parseL31CExp(elseClause));

    return bind(clausesResult, (clauses: CondClause[]) => 
            bind(parsedelseClauseResult, (elseClause: CExp) => makeOk(makeCondExp(clauses, makeElseClause(elseClause)))));
}

const parseElseExp = (params: Sexp): Result<Sexp> => {
    if(isArray(params) && params.length === 2 && params[0] === 'else'){
        return makeOk(params[1]);
    }
    else{
        return makeFailure("Invalid else clause");
    }
...
const unparseCondExp = (ce: CondExp) : string => 
    `(cond ${map((c: CondClause) => `(${unparseL31(c.test)} ${unparseL31(c.then)})`, ce.condClauses).join(" ")} (else ${unparseL31(ce.elseClause.alt)}))`;
