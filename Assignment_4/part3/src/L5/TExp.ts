...
*/
import { chain, concat, map, uniq, equals} from "ramda";
...
export const makeUnionTExp = (components: TExp[]): UnionTExp => ({tag: "UnionTExp", components: uniq(components)});
...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> => {
    ...
    return (texps[0] === 'union') ? parseUnionTExp(texps.slice(1)) :
...
export const makeUnionTE = (left: TExp, right: TExp): TExp => {

    const makeAtomicCompondUnion = (left: AtomicTExp, right: CompoundTExp): TExp =>{
        if(isUnionTExp(right)) {
            if (!right.components.some(te => te.tag === left.tag)) {
                        right.components.push(left);
                        right.components.sort((a, b) => a.tag.localeCompare(b.tag));
                        return right;
                    }
                    else {
                        return right;
            ...
            return makeUnionTExp([left, right].sort((a, b) => a.tag.localeCompare(b.tag)));
        }
        
    };
    const makeCompoundUnion = (left: CompoundTExp, right: CompoundTExp): TExp =>{
        if (isUnionTExp(left) && isUnionTExp(right)) {
            const mergedComponents = uniq(left.components.concat(right.components));
            mergedComponents.sort((a, b) => a.tag.localeCompare(b.tag));
            return makeUnionTExp(mergedComponents);
        }
        else if(isUnionTExp(left) && isProcTExp(right)) {
            left.components.push(left);
            left.components.sort((a, b) => a.tag.localeCompare(b.tag));
            return left;
        }
        else if(isProcTExp(left) && isUnionTExp(right)) {
            right.components.push(left);
            right.components.sort((a, b) => a.tag.localeCompare(b.tag));
            return right;
        }
        else if(isProcTExp(left) && isProcTExp(right)){
            return equals(left.paramTEs, right.paramTEs) && equals(left.returnTE, right.returnTE) ? left : 
                        makeUnionTExp([left, right]);
        }
        else {
            return makeUnionTExp([left, right].sort((a, b) => a.tag.localeCompare(b.tag)));
        }
    };
    const makeAtomicTvarUnion = (left: AtomicTExp, right: TVar): TExp =>{
        return makeUnionTExp([left, right].sort((a, b) => a.tag.localeCompare(b.tag)));
    };
    const makeCompundTvarUnion = (left: CompoundTExp, right: TVar): TExp => {

        const isDuplicateTvarInUnion = (left: UnionTExp, right: TVar): boolean => 
                left.components.filter(te => isTVar(te) && eqTVar(te , right)).length > 0;
            
        
        if(isUnionTExp(left)) {
            if (!isDuplicateTvarInUnion(left, right)) {
                        left.components.push(left);
                        left.components.sort((a, b) => a.tag.localeCompare(b.tag));
                        return left;
            }
    ...
    const makeTvarUnion = (left: TVar, right: TVar): TExp => eqTVar(left, right) ? left :
            makeUnionTExp([left, right].sort((a, b) => a.var.localeCompare(b.var)));
    

    return  (isAtomicTExp(left) && isAtomicTExp(right)) ? eqAtomicTExp(left, right) ? left : makeUnionTExp([left, right].sort((a, b) => a.tag.localeCompare(b.tag))):
            (isAtomicTExp(left) && isCompoundTExp(right)) ? makeAtomicCompondUnion(left, right) :
            (isCompoundTExp(left) && isAtomicTExp(right)) ? makeAtomicCompondUnion(right, left) :
            (isCompoundTExp(left) && isCompoundTExp(right)) ? makeCompoundUnion(left, right) :
            (isAtomicTExp(left) && isTVar(right)) ? makeAtomicTvarUnion(left, right) :
            (isTVar(left) && isAtomicTExp(right)) ? makeAtomicTvarUnion(right, left) :
            (isCompoundTExp(left) && isTVar(right)) ? makeCompundTvarUnion(left, right) :
            (isTVar(left) && isCompoundTExp(right)) ? makeCompundTvarUnion(right, left) :
            (isTVar(left) && isTVar(right)) ? makeTvarUnion(left, right):
            left;
};

const parseUnionTExp = (texps: Sexp[]): Result<TExp> => bind(parseTExp(texps[0]) , (first: TExp) => 
        bind(parseTExp(texps[1]) , (second: TExp) => makeOk(makeUnionTE(first, second))));
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const unparseUnion = (compTEs: TExp[]): Result<string> => 
        compTEs.length < 2 ? makeFailure("Union with less than 2 components") :
        compTEs.length === 2 ? bind(unparseTExp(compTEs[0]), (texp1: string) => 
                                    bind(unparseTExp(compTEs[1]), (texp2: string) =>
                                        makeOk( `(union ${texp1} ${texp2})` ))) :
        compTEs.length > 2 ? bind(unparseTExp(compTEs[0]), (unparsed0: string) =>
                                bind(unparseUnion(compTEs.slice(1)), (unparsedRest: string) =>
                                        makeOk(`(union ${unparsed0} ${unparsedRest})`))):
        makeFailure("Invalid Union");
