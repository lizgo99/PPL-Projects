...
import { cons, first, second } from "../src/shared/list";
import { TExp, isProcTExp, isUnionTExp, parseTE, parseTExp, unparseTExp } from "../src/L5/TExp";
...
const pt = (x: string): Result<TExp> => bind(parseSexp(x), (s) => parseTExp(s));
...
describe('L5 parseTExp Union Parser', () => {
    ...
    it('parseTExp parses embedded union expressions', () => {
        expect(pt("(union boolean (union number string))")).toSatisfy(isOkT(isUnionTExp));
    });

    it('parseTExp parses union types in proc argument position', () => {
        expect(pt("((union number string) -> boolean)")).toSatisfy(isOkT(isProcTExp));
    });

    it('parseTExp parses union types in return type argument position', () => {
        expect(pt("((union number string) -> (union number string))")).toSatisfy(isOkT(isProcTExp));
    });

    it('parseTExp fails to parse union of bad type expressions', () => {
        expect(pt("(union number)")).toSatisfy(isFailure);
    });
...
describe('L5 parse with unions', () => {
    ...
    it('unparses union of atomic types in different positions: define, let, param, return types', () => {
        const dt1 = `
        (L5 
            (define x : (union number boolean) 5)
            (let ((y : (union number boolean) 10))
                (+ x y))
            (define (f x : (union number boolean)) : (union number boolean)
                (if x
                    5
                    #t))
        )
    ...
    it('parses nested union expressions', () => {
        const dt2 = `
        (L5 
            (define f (lambda ((x (union number boolean))) : (union number (union string boolean))
                (if (number? x)
                    (if (< x 0)
                        #f
                        (* x x)
...
describe('L5 Unparse', () => {
    ...
    it('unparses union, nested unions in different positions', () => {
        const nestedUnions = "(define (x : (union boolean (union number string))) 5)";
        expect(roundTrip(nestedUnions)).toEqual(makeOk(nestedUnions));
    })
});
