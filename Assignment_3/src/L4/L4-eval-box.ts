...
import { isClosure, makeClosure, Closure, Value, makeCompoundSExp, CompoundSExp, isSExp } from "./L4-value-box";
import { applyPrimitive } from "./evalPrimitive-box";
import { first, rest, isEmpty, isNonEmptyList, cons } from "../shared/list";
...
const evalBound = (exp: BoundExp, env: Env): Result<boolean> =>
    (isFailure(applyEnv(env, exp.var.var))) ? makeOk(false) : makeOk(true);
...
const evalTime = (exp: TimeExp, env: Env): Result<CompoundSExp> => {
    ...
    const end = Date.now();
    return isFailure(val) ? val : makeOk(makeCompoundSExp(val.value, end - start));
}
