...
export const makeTimeExp = (cexp: CExp): TimeExp =>
    ({tag: "TimeExp", exp: cexp});
...
export const parseBoundExp = (params: Sexp[]): Result<BoundExp> =>
    isNonEmptyList<Sexp>(params) ?
    (params.length === 1) ? parseGoodBoundExp(first(params)) :
        makeFailure("illegal number of arguments - can't be more than 1 expression") :
    makeFailure("missing an argument");

export const parseGoodBoundExp = (variable: Sexp): Result<BoundExp> => 
    ! isIdentifier(variable) ? makeFailure(`First arg of bound? must be an identifier: ${format(variable)}`) :
    makeOk(makeBoundExp(makeVarRef(variable)));
    
export const parseTimeExp = (params: Sexp[]): Result<TimeExp> => 
    isNonEmptyList<Sexp>(params) ?
        (params.length === 1) ? bind(parseL4CExp(first(params)), makeTimeResult) :
            makeFailure("illegal number of arguments - can't be more than 1 expression"):
    makeFailure("missing an argument");

export const makeTimeResult = (exp: CExp): Result<TimeExp> =>
    makeOk(makeTimeExp(exp));
...
const unparseBoundExp = (be: BoundExp): string =>
    `(bound? ${be.var.var})`;
    
const unparseTimeExp = (te: TimeExp): string =>
    `(time ${unparse(te.exp)})`;
