...
(define equal-trees$ 
  (lambda (tree1 tree2 succ fail)
    (cond
      ((and (empty-tree? tree1) (empty-tree? tree2)) (succ '()))
      ((or (empty-tree? tree1) (empty-tree? tree2)) (fail (cons tree1 tree2)))
      ...
      (else 
        (equal-trees$ (first-subtree tree1) (first-subtree tree2)
                      (lambda (car-res) (equal-trees$ (rest-subtree tree1) (rest-subtree tree2)
                              (lambda (cdr-res) (succ (cons car-res cdr-res)))
                                      fail))
                      fail)
      )
    )
...
(define as-real
  (lambda (x)
    (define real-lzl (cons-lzl x (lambda () real-lzl)))
    real-lzl
  )
)
...
(define sqrt-with
  (lambda (x y)
    (define lzl-y-squared (** y y))
    (define lzl-y-squared+x (++ x lzl-y-squared))
    (define lzl-2y (++ y y))
    (define lzl-y-squared+x/2y (// lzl-y-squared+x lzl-2y))
    (cons-lzl y (lambda () (sqrt-with x lzl-y-squared+x/2y)))
  )
) 
...
(define diag-help
  (lambda (lzl index)
    (cons-lzl (nth (head lzl) (- index 1)) 
      (lambda () 
        (diag-help (tail lzl) (+ index 1)) 
      )
    )
  )
)
